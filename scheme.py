...
def eval_all(expressions, env):
    ...
    if expressions is nil: 
        ...
        #recursively evaluates the list.
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN Question 4
        args = list(args) # Convert a Scheme list to a Python list
        if self.use_env:
            args.append(env)
        try:
            return self.fn(*args)
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN Question 12
        if isinstance(self, LambdaProcedure): 
            return self.env.make_child_frame(self.formals, args)
        elif isinstance(self, MuProcedure):
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        expr_val = scheme_eval(expr, env, last)
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN Question 17
    def make_call_frame(self, args, env): #question 12
        """Make a frame that binds the my formal parameters to ARGS, a
        Scheme list of values, for a call evaluated in environment env."""
        if isinstance(self, LambdaProcedure): 
            return self.env.make_child_frame(self.formals, args)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    if tail:
        # BEGIN Extra Credit
        return Evaluate(expr, env) #keeps giving me recursion error despite changing all tail calls of scheme_eval with True as the third argument 
        # END Extra Credit
    else:
